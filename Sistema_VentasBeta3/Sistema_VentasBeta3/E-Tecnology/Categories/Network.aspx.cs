...
namespace Sistema_VentasBeta3.E_Tecnology.Categories
{
    public partial class Network : System.Web.UI.Page
    {
        ...
        protected void Cargar_Bluetooth()
        {
            SqlConnection conex = new SqlConnection(ConexString);
            conex.Open();
            SqlCommand cmd = new SqlCommand("SP_SearchBluetooth", conex);
        ...
        protected void Cargar_Switch()
        {
            SqlConnection conex = new SqlConnection(ConexString);
            conex.Open();
            SqlCommand cmd = new SqlCommand("SP_SearchSwitch", conex);
            cmd.CommandType = CommandType.StoredProcedure;
            SqlDataReader rd = cmd.ExecuteReader(CommandBehavior.CloseConnection);
            DataTable tb = new DataTable();
            tb.Load(rd);
